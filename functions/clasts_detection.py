...
def clasts_detect(mode, imgpath, resolution=0.001, metric_cropsize=1, plot=True, saveplot=False, saveresults=False, devicemode="gpu", devicenumber=0, n_zero=0):
...
#                 ax_list=np.zeros([n_detected_objects,2])
    ...
    if str.lower(mode)=="uav":
        ...
        for n in range(n_zero, int(shpimg[0]/cropsize)+1): #Lignes
            for m in range(0, int(shpimg[1]/cropsize)+1): #Colonnes
                ...
                if (pixel_value>0):
                    ...
                    print('---------------------------------------------------------')
                    print('Processing crops: '+'n = '+str(int(n))+', m = '+str(int(m))+' - '+str(np.round((((n*int(shpimg[1]/cropsize))+(m+1))/n_crops)*100,3))+' % - '+str((n*int(shpimg[1]/cropsize))+(m+1))+' / '+str(n_crops)+' crops processed')
            ...
            if saveresults==True:
                ...
                clasts_temp.to_csv(imgpath[0:-4]+'_checkpoint_save_n='+str(n)+'_window_size='+str(metric_cropsize)+'m_individual_clast_values.csv', index=False)
                print(imgpath[0:-4]+'_checkpoint_save_n='+str(n)+'_window_size='+str(metric_cropsize)+'m_individual_clast_values.csv')
